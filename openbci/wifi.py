...
class OpenBCIWiFi(object):
    ...
    def start_streaming(self, callback, lapse=-1):
        ...
        self.delay = self.timer_end-self.timer_start
    ...
    def save_record(self):
        ...
        self.local_wifi_server.handler.records[0][0] = self.delay
        np.save('recording_0', self.local_wifi_server.handler.records)
        ...
        if len(data) > 2:
            if self.high_speed:
                ...
                for sample in samples:
                    ...
                    # print(sample.sample_number, self.last_odd_sample.sample_number, np.array(sample.channel_data))
                    if self.daisy:
                        # odd sample: daisy sample, save for later
                        if self.first or sample.sample_number != self.last_odd_sample.sample_number:
                        ...
                        # even sample: concatenate and send if last sample was the first part, otherwise drop the packet
                            ...
                            print('daisy',sample.sample_number,sample.channel_data)
